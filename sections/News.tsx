  ...
  const listItems = [
    {
      ...
      title: "THE LINE PARTNERS WITH RIOT GAMES ON LEAGUE OF LEGENDS MUSIC VIDEO",
      description: {
        desktop: [
          <Fragment key="d1">We are proud to unveil our latest collaboration with Riot Games: a breathtaking</Fragment>,
          <Fragment key="d2">three-minute music video to celebrate the release of a brand-new set of Spirit</Fragment>,
    ...
    },
    {
      ...
      description: {
        desktop: [<Fragment key="d1">Our limited edition store! Something we've been working on for a longggg time.</Fragment>],
        mobile: "Our limited edition store! Something we've been working on for a longggg time.",
      },
    },
  ];
  return (
      ...
      <div className="mt-[158px] mb-[15px] lg:mt-[18.51852vw] lg:mb-[1.04167vw]">
        <Button href="https://thelinestudio.com/blog">
          <span className="relative">
            See All
            <span className="absolute top-0 left-full ml-[3px] text-[0.70vw] leading-[1.75] font-[440] tracking-[-.01em]">[&nbsp;10&nbsp;]</span>
          </span>
        </Button>
      </div>
      <div className="flex flex-col gap-[120px] lg:flex-row lg:gap-[0.46296vw]">
        {listItems.map((eachListItem, index) => (
          <News.NewsListItem key={"list-item-" + (index + 1)} {...eachListItem} />
        ))}
      </div>
    </div>
  );
...
News.NewsListItem = ({
  ...
  <Link href={href} className="flex-1 cursor-pointer">
    <motion.div initial="initial" whileHover="whileHover" className="flex-1 cursor-pointer">
      <div className="relative mb-[5px] aspect-[390/249] max-lg:-mx-2 lg:mb-[.23148vw]">
        <Image src={imgSrc} alt={title} className="h-full w-full object-cover" />
        <CursorPlane className="text-white">read more</CursorPlane>
      </div>
      <Label>{label}</Label>
      <p className="py-2.5 text-[32px] leading-[0.85] font-[440] tracking-[-0.03em] lg:max-w-[80%] lg:py-[0.46296vw] lg:text-[2.31481vw]">{title}</p>
